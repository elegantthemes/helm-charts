...
First apply of this chart against today’s one-pod app: Slack/Discord/webhooks are down until the intake pods are Ready (fetch/build). After that, intake rolling deploys keep an old Ready replica until the new one is Ready. Consumer deploys drain in-flight fix/feedback jobs (grace 1800s).
